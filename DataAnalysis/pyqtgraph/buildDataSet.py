...
def setListItems(pathLogNr):
    ...
    for name, timeIndex, dataIndex, fileName, vis, sig, wid, order, scale in groups:
        ...
        kartData[name]['info'] = [vis, sig, wid, order, scale]#item.info = [visible, filter_sigma, filter_width, order, scale]
...
def preProcessing(kartData, name):
    ...
    if name in differentiate:
        ...
        t, dydt = prep.derivative_X_dX(name, kartData[nameFrom]['data'][0], kartData[nameFrom]['data'][1])
