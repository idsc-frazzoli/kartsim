...
# model_name = 'poly2_order3'
# model_name = 'poly2_sparse'
# model_name = '0x99_None_reg0p0001_50ksample_expotrigopoly3reduced_l1sparse_axonly_directinput'
# model_name = '0x99_None_reg0p0001_50ksample_expotrigopoly3reduced_l1sparse_ayonly_directinput'
# model_name = '0x99_None_reg0p0001_50ksample_expotrigopoly3reduced_l1sparse_athetaonly_directinput'
# model_name = '1x64_tanh_reg0p0001_nomodel_directinput'
# model_name = '1x16_tanh_reg0p0_nomodel_morefeatures_from_poly3reduced_directinput'
# model_name = '1x16_tanh_reg0p0_nomodel_ayonly_directinput_test_mlpnormal'
...
# model_name = '1x16_softplus_reg0p0001_nomodel_directinput_fulldata_mlpsymmetric'
# model_name = '1x16_tanh_reg0p0_kin_directinput_fulldata_mlpsymmetric'

# model_name = '0x6_None_reg0p005_dyn_directinput_fulldata_mlpsymmetric'
model_name = '0x6_None_reg0p001_dyn_directinput_fulldata_mlpsymmetric'
...
if not 'sparse' in model_name:
    ...
    # mlp.load_checkpoint(checkpoint=-100)
...
for i, (w, b) in enumerate(zip(weights, biases)):
    ...
    #     res_list = np.array([(indizes[0], weights[indizes[0]], mlp.train_stats.index.values[indizes[0]])], dtype=[('index', int), ('score', np.float), ('name', 'U100')])
    #     equation = 'x = '
    #     equation += f'{weights[indizes[0]]} * ({mlp.train_stats.index.values[indizes[0]]} - {means[indizes[0]]}) / {stds[indizes[0]]} + '
    #     print_str = 'indizes ['
    #     print_str += f'{indizes[0]}, '
    #     for index in indizes[1:]:
    #         print_str += f'{index}, '
    #         equation += f'{weights[index]} * ({mlp.train_stats.index.values[index]} - {means[index]}) / {stds[index]} + '
    #         res_list = np.vstack((res_list, np.array([(index, weights[index], mlp.train_stats.index.values[index])], dtype=[('index', int), ('score', np.float), ('name', 'U100')])))
    #     print(print_str, ']')
...
# model = DataDrivenVehicleModel(model_type='no_model', model_name=model_name, direct_input=True)
# # model = NoModelSparseModel(model_name=model_name)
# model = NoModelModel(model_name=model_name, direct_input=True)
inputs = [5.0,-0.1,0.5,0.25,-1.0,0.5] #[-1.53183965 -4.04873158 -8.52437451]
