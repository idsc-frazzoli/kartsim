...
def main():
    ...
    while True:
        try:
            if 'mirrored' in fileNames[fileNameIndex]:
                savePathName = os.path.join(savePath, fileNames[fileNameIndex][:18] +
                                              '_mirrored_{}_closedloop.csv'.format(model_log_name))
            else:
                savePathName = os.path.join(savePath, fileNames[fileNameIndex][:18] +
    ...
    if X.shape[0] != 0 and X.shape[1] != 0:
        ...
        dataFrame.to_csv(save_path, index=False,
                         header=['time [s]', 'pose x [m]', 'pose y [m]', 'pose theta [rad]', 'vehicle vx [m*s^-1]',
                                 ...
                                 'pose atheta [rad*s^-2]', 'turning angle [n.a]', 'acceleration rear axle [m*s^-2]',
