...
def main():
    ...
    validation = True
    validationhorizon = 3  # [s] time inteval after which initial conditions are reset to values from log data
...
def execute_simulation(conn, _wait_for_real_time, sim_time, sim_time_increment, server_return_interval, data_time_step,
    ...
    U = np.array((preprodata['time [s]'][0:firstStep].values,
                  ...
                  preprodata['acceleration torque vectoring [rad*s^-2]'][0:firstStep].values))
    ...
    while not ticker.wait(_wait_for_real_time):
        if i >= i_max:
            ...
            U = np.vstack((preprodata['time [s]'][simRange[0]:simRange[1]].values,
                           preprodata['turning angle [n.a]'][simRange[0]:simRange[1]].values,
                           preprodata['acceleration rear axle [m*s^-2]'][simRange[0]:simRange[1]].values,
                           preprodata['acceleration torque vectoring [rad*s^-2]'][simRange[0]:simRange[1]].values))
