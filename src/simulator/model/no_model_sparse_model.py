...
class NoModelSparseModel:
    def __init__(self, model_type='mlp', model_name='FirstTry'):
        ...
        elif model_name == 'poly3_sparse':
            self.predictor = self.NN_poly3_sparse #unstable
        elif model_name == 'poly3reduced_sparse':
            self.predictor = self.NN_poly3reduced_sparse #unstable
        ...
        elif model_name == 'poly2_order14':
            self.predictor = self.NN_poly2_order14  # stable, but unrealistic cornering/drifting
        elif model_name == 'poly2_sparse':
            self.predictor = self.NN_poly2_sparse #unrealistic cornering/drifting and unstable if vy =! 0 and vx =0
    ...
    def NN_poly3_sparse(self, features):
        if isinstance(features, list):
            VX, VY, VTHETA, BETA, AB, TV = features[0]
            VX, VY, VTHETA, BETA, AB, TV = float(VX), float(VY), float(VTHETA), float(BETA), float(AB), float(TV)
        else:
            ...
            AB = features[:, 4]
            TV = features[:, 5]
        AX = 0.14598597586154938 * (sin(AB/6.7*pi/2.0) - 0.017419083821411975) / 0.1785014463024927 + 0.13383236527442932 * (tan(AB/6.7*pi/3.0) - 0.00824770448780333) / 0.14978557053838454 + 0.25861233472824097 * ((np.power(2,AB)) * (cos(AB/6.7*pi/2.0)) - 1.1791048903469692) / 0.6382375070463543 + 0.12330932915210724 * ((VY) * (VTHETA) * (cos(VX/10.0*pi/2.0)) - 0.011614177751649895) / 0.2979429797821768 + 0.20195002853870392 * ((VY) * (VTHETA) * (cos(AB/6.7*pi/2.0)) - -0.0034241363837385448) / 0.4297752996153552 + 0.10807055234909058 * ((VY) * (VTHETA) * (tan(VX/10.0*pi/3.0)) - -0.028075337879231933) / 0.3215799282965345 + 0.1221790760755539 * ((AB) * (cos(BETA/0.44*pi/2.0)) * (cos(TV/2.1*pi/2.0)) - 0.06667145440980085) / 0.6568028772547091 + 0.07450488209724426 * ((sin(AB/6.7*pi/2.0)) * (sin(AB/6.7*pi/2.0)) * (sin(AB/6.7*pi/2.0)) - -0.004793138828149807) / 0.07675413857698866 + 0.10743363201618195 * ((sin(TV/2.1*pi/2.0)) * (cos(BETA/0.44*pi/2.0)) * (tan(BETA/0.44*pi/3.0)) - 0.0927588623544914) / 0.1149336931829428 + -0.11897193640470505 * ((np.power(1/2.0,VX)) * (np.power(1/2.0,VX)) * (tan(AB/6.7*pi/3.0)) - 1.4022951669001227e-05) / 0.0359719031099005 + -0.11755716055631638 * ((np.power(1/2.0,AB)) * (cos(AB/6.7*pi/2.0)) * (cos(AB/6.7*pi/2.0)) - 1.0358406207896826) / 0.6494655629004047 + -0.16068801283836365 * ((sin(VX/10.0*pi/2.0)) * (sin(BETA/0.44*pi/2.0)) * (sin(BETA/0.44*pi/2.0)) - 0.13253847684534972) / 0.15639161518495984 + 0.0030376880895346403

        AY = 0.175088569521904 * ((VX) * (cos(VX/10.0*pi/2.0)) * (tan(BETA/0.44*pi/3.0)) - -0.12901701015276035) / 1.3220833211736505 + 0.3208964765071869 * ((VY) * (VTHETA) * (VTHETA) - 0.026297031571554143) / 0.8483148589544969 + 0.18700358271598816 * ((VY) * (np.power(1/2.0,VX)) * (cos(AB/6.7*pi/2.0)) - 0.0007889697481158796) / 0.04030734394774132 + 0.3569851219654083 * ((VTHETA) * (cos(VX/10.0*pi/2.0)) * (cos(VX/10.0*pi/2.0)) - -0.030270735736605683) / 0.3437613897979791 + 0.2635744512081146 * ((BETA) * (sin(VX/10.0*pi/2.0)) * (cos(TV/2.1*pi/2.0)) - -0.00658344568162781) / 0.06916596921843254 + -0.27597251534461975 * ((VY) * (cos(VX/10.0*pi/2.0)) * (cos(VX/10.0*pi/2.0)) - -0.004006374758909815) / 0.17575600464338803 + -0.2782064974308014 * ((VY) * (cos(VX/10.0*pi/2.0)) * (cos(AB/6.7*pi/2.0)) - -0.003290577280095432) / 0.21026023128849894 + -0.1415778547525406 * ((VY) * (cos(AB/6.7*pi/2.0)) * (cos(TV/2.1*pi/2.0)) - 0.001228621718865396) / 0.24719584038196304 + -0.30473265051841736 * ((VTHETA) * (VTHETA) * (VTHETA) - -0.06823344395446589) / 1.1956058156183735 + -0.15191224217414856 * ((VTHETA) * (sin(VX/10.0*pi/2.0)) * (sin(VX/10.0*pi/2.0)) - -0.01775611810273919) / 0.2779917650152217 + -0.25153210759162903 * ((np.power(2,VX)) * (cos(VX/10.0*pi/2.0)) * (tan(BETA/0.44*pi/3.0)) - -0.6083722018073039) / 9.481076711683418 + -0.2114114910364151 * ((np.power(1/2.0,VX)) * (sin(VX/10.0*pi/2.0)) * (sin(BETA/0.44*pi/2.0)) - -0.004947157530257157) / 0.03760730698282643 + 0.0011673446279019117

        ATHETA = 0.43170568346977234 * ((VX) * (BETA) * (cos(VX/10.0*pi/2.0)) - -0.044562332862094906) / 0.4477261272339822 + 0.2783956229686737 * ((VX) * (sin(BETA/0.44*pi/2.0)) * (cos(AB/6.7*pi/2.0)) - -0.17553810801460182) / 1.772763941713415 + 0.4171558916568756 * ((VX) * (cos(VX/10.0*pi/2.0)) * (tan(BETA/0.44*pi/3.0)) - -0.12901701015276035) / 1.3220833211736505 + 0.4692646563053131 * ((VTHETA) * (VTHETA) * (VTHETA) - -0.06823344395446589) / 1.1956058156183735 + 0.304768830537796 * ((BETA) * (sin(VX/10.0*pi/2.0)) * (cos(BETA/0.44*pi/2.0)) - -0.004660552573646553) / 0.04956558064052531 + 0.3095383048057556 * ((TV) * (sin(VX/10.0*pi/2.0)) * (sin(VX/10.0*pi/2.0)) - -0.018286674049614825) / 0.2557357204626517 + -0.3077561855316162 * ((VTHETA) * (cos(VX/10.0*pi/2.0)) - -0.03558960931344437) / 0.41623512135178375 + -0.38212165236473083 * ((VTHETA) * (cos(AB/6.7*pi/2.0)) - -0.047068208364958584) / 0.5338412523843211 + -0.2686748206615448 * ((VX) * (VTHETA) * (cos(TV/2.1*pi/2.0)) - -0.13879763353573785) / 2.1882408757607976 + -0.34326839447021484 * ((VTHETA) * (sin(BETA/0.44*pi/2.0)) * (tan(BETA/0.44*pi/3.0)) - -0.035340674062953874) / 0.4419165143772196 + -0.27689775824546814 * ((VTHETA) * (cos(VX/10.0*pi/2.0)) * (cos(VX/10.0*pi/2.0)) - -0.030270735736605683) / 0.3437613897979791 + -0.4086662828922272 * ((VTHETA) * (cos(BETA/0.44*pi/2.0)) * (cos(AB/6.7*pi/2.0)) - -0.024533058035904626) / 0.3227320389025673 + 0.0014749490655958652
    ...
    def NN_poly3reduced_sparse(self, features):
        if isinstance(features, list):
            VX, VY, VTHETA, BETA, AB, TV = features[0]
            VX, VY, VTHETA, BETA, AB, TV = float(VX), float(VY), float(VTHETA), float(BETA), float(AB), float(TV)
        else:
            ...
            AB = features[:, 4]
            TV = features[:, 5]
        AX = 0.37226736545562744 * (AB - 0.06560439433747117) / 0.837672127818587 + 0.16241060197353363 * (np.power(1/2.0,AB) - 1.373170976826475) / 4.343141970106426 + 0.24968092143535614 * (tan(AB/6.7*pi/3.0) - 0.00824770448780333) / 0.14978557053838454 + 0.556675136089325 * ((VX) * (AB) - 0.4213669524546223) / 4.251815853244271 + 0.3911014795303345 * ((VY) * (VTHETA) - -0.0075517562606167935) / 0.4547007200691509 + 0.09293395280838013 * ((VTHETA) * (TV) - 0.24997696696256436) / 0.4690375873163249 + -0.18810875713825226 * (cos(BETA/0.44*pi/2.0) - 0.7617875788004159) / 0.2510784486857777 + -0.0674312487244606 * ((VTHETA) * (VTHETA) - 0.30753697144650266) / 0.6347497015012956 + -0.1579507738351822 * ((BETA) * (BETA) - 0.04121559965423868) / 0.04580274658393033 + -0.3922823667526245 * ((VX) * (VX) * (AB) - 2.830314122401715) / 29.274345670034155 + -0.14021050930023193 * ((VX) * (BETA) * (BETA) - 0.10322624822347427) / 0.14699888695062427 + 0.0030841592233628035

        AY = 0.6697922945022583 * (VTHETA - -0.04802685383934488) / 0.5524821241332127 + 0.07752442359924316 * ((VX) * (TV) - -0.2317972077113321) / 2.6490140417448003 + 0.2915951609611511 * ((VX) * (VX) * (VY) - 0.3681228086413301) / 10.242007534778573 + 0.1131596639752388 * ((VX) * (VTHETA) * (AB) - 0.22140526576970496) / 3.972996150420444 + 0.18777543306350708 * ((VY) * (VTHETA) * (VTHETA) - 0.026297031571554143) / 0.8483148589544969 + -0.6667926907539368 * (VY - 0.0006739826858158322) / 0.3073995994274385 + -0.09354855120182037 * ((VX) * (VY) - 0.03187135437735826) / 1.6143434321121914 + -0.44417843222618103 * ((VX) * (VX) * (VTHETA) - -1.1304472770254048) / 16.745791698795607 + -0.12385836988687515 * ((VX) * (VX) * (TV) - -1.0850356721033614) / 15.725026610310815 + -0.0930498018860817 * ((VY) * (VY) * (VTHETA) - -0.035928699026757015) / 1.0611067890823749 + -0.346895307302475 * ((VTHETA) * (VTHETA) * (VTHETA) - -0.06823344395446589) / 1.1956058156183735 + 0.0009479235741309822

        ATHETA = 1.185070514678955 * ((VX) * (BETA) - -0.059164368850320524) / 0.616404092345298 + 0.35904166102409363 * ((VX) * (TV) - -0.2317972077113321) / 2.6490140417448003 + 0.4281342923641205 * ((VX) * (VX) * (VY) - 0.3681228086413301) / 10.242007534778573 + 0.26844632625579834 * ((VX) * (VX) * (BETA) - -0.22996882531985796) / 3.2498046086031787 + 0.18142597377300262 * ((VX) * (BETA) * (AB) - 0.03662337538062741) / 0.7940023371395283 + 0.19570960104465485 * ((VY) * (BETA) * (BETA) - -0.00011789782511803328) / 0.024391213816947373 + 0.4043024480342865 * ((VTHETA) * (VTHETA) * (VTHETA) - -0.06823344395446589) / 1.1956058156183735 + -1.5047032833099365 * (VTHETA - -0.04802685383934488) / 0.5524821241332127 + -0.5413983464241028 * ((VX) * (VY) - 0.03187135437735826) / 1.6143434321121914 + -0.38907867670059204 * ((VX) * (VTHETA) - -0.19631195879670152) / 2.706649563739692 + -0.27236616611480713 * ((VX) * (VTHETA) * (AB) - 0.22140526576970496) / 3.972996150420444 + 0.0007847734377719462
    ...
    def NN_poly2_order14(self, features):
        if isinstance(features, list):
            VX, VY, VTHETA, BETA, AB, TV = features[0]
            VX, VY, VTHETA, BETA, AB, TV = float(VX), float(VY), float(VTHETA), float(BETA), float(AB), float(TV)
        else:
            ...
            AB = features[:, 4]
            TV = features[:, 5]
        AX = 0.34051820635795593 * (AB - 0.06560439433747117) / 0.837672127818587 + 0.4218772053718567 * (np.power(2,AB) - 1.2063352407160353) / 0.7105174540771907 + 0.207598015666008 * (cos(AB/6.7*pi/2.0) - 0.9815492593309374) / 0.06629921463089376 + 0.20048788189888 * ((VX) * (cos(BETA/0.44*pi/2.0)) - 2.086206577040916) / 1.7625891197618575 + 0.16405710577964783 * ((VX) * (cos(TV/2.1*pi/2.0)) - 2.2877517295054317) / 1.6888098448611537 + 0.4354836642742157 * ((VY) * (VTHETA) - -0.0075517562606167935) / 0.4547007200691509 + 0.20601442456245422 * ((TV) * (tan(AB/6.7*pi/3.0)) - 0.004208543757604824) / 0.08571510690729939 + 0.16823305189609528 * ((np.power(2,AB)) * (cos(BETA/0.44*pi/2.0)) - 0.9252905644261366) / 0.6732057133840967 + 0.1296985000371933 * ((np.power(1/2.0,AB)) * (cos(TV/2.1*pi/2.0)) - 1.2103699084418131) / 4.087102263612652 + 0.38054320216178894 * ((sin(AB/6.7*pi/2.0)) * (cos(TV/2.1*pi/2.0)) - 0.02203966103905764) / 0.1666718588702699 + 0.153880313038826 * ((cos(VX/10.0*pi/2.0)) * (tan(AB/6.7*pi/3.0)) - 0.003675304873146799) / 0.1154323813869788 + 0.22138047218322754 * ((cos(BETA/0.44*pi/2.0)) * (cos(AB/6.7*pi/2.0)) - 0.7472803531682797) / 0.2511043678086817 + -0.23081938922405243 * (tan(AB/6.7*pi/3.0) - 0.00824770448780333) / 0.14978557053838454 + -0.24990959465503693 * ((np.power(2,AB)) * (cos(TV/2.1*pi/2.0)) - 1.1059169148482064) / 0.7502383658131054 + -0.2043594866991043 * ((AB) * (np.power(1/2.0,VX)) - 0.002780417668569421) / 0.2644823427681953 + -0.18174268305301666 * ((np.power(2,AB)) * (sin(VX/10.0*pi/2.0)) - 0.5247879826545464) / 0.6663679076748461 + -0.1509644240140915 * ((sin(VX/10.0*pi/2.0)) * (cos(AB/6.7*pi/2.0)) - 0.3759059155489542) / 0.22705908086289384 + -0.1940753161907196 * ((sin(TV/2.1*pi/2.0)) * (tan(AB/6.7*pi/3.0)) - 0.002685427844477961) / 0.05368985926107068 + -0.12360712885856628 * ((cos(VX/10.0*pi/2.0)) * (cos(BETA/0.44*pi/2.0)) - 0.6621784366881095) / 0.2555961572100748 + -0.2007758766412735 * ((cos(BETA/0.44*pi/2.0)) * (cos(TV/2.1*pi/2.0)) - 0.6944144109841824) / 0.283880552003958 + -0.261525958776474 * ((cos(AB/6.7*pi/2.0)) * (tan(VX/10.0*pi/3.0)) - 0.2988186821761009) / 0.2425852112265329 + 0.003202243475243449

        AY = 0.2089015245437622 * ((VX) * (VY) - 0.03187135437735826) / 1.6143434321121914 + 0.21346579492092133 * ((VY) * (tan(VX/10.0*pi/3.0)) - 0.005144678661245813) / 0.20226677927976314 + 0.21341609954833984 * ((VTHETA) * (np.power(2,AB)) - -0.0232559976392174) / 0.9453828498950136 + 0.32159262895584106 * ((VTHETA) * (cos(VX/10.0*pi/2.0)) - -0.03558960931344437) / 0.41623512135178375 + 0.1497839093208313 * ((BETA) * (cos(TV/2.1*pi/2.0)) - -0.023337366230351705) / 0.17231109199124836 + 0.13758927583694458 * ((sin(VX/10.0*pi/2.0)) * (sin(BETA/0.44*pi/2.0)) - -0.02551175347595281) / 0.2586397131882021 + 0.21976275742053986 * ((sin(TV/2.1*pi/2.0)) * (cos(VX/10.0*pi/2.0)) - -0.04287448217319001) / 0.3447850390439351 + 0.11090455204248428 * ((cos(BETA/0.44*pi/2.0)) * (tan(TV/2.1*pi/3.0)) - -0.028359014070580238) / 0.2458638828706388 + 0.2906903028488159 * (VY - 0.0006739826858158322) / 0.3073995994274385 + 0.2785928249359131 * ((BETA) * (tan(VX/10.0*pi/3.0)) - -0.006927802459959079) / 0.07331567891700316 + -0.14480534195899963 * ((VY) * (sin(VX/10.0*pi/2.0)) - 0.0030753815466207056) / 0.21484197523316045 + -0.35247543454170227 * ((sin(VX/10.0*pi/2.0)) * (tan(BETA/0.44*pi/3.0)) - -0.02448072465955652) / 0.2542822057050383 + -0.10916818678379059 * ((VX) * (VTHETA) - -0.19631195879670152) / 2.706649563739692 + -0.179949089884758 * ((VX) * (TV) - -0.2317972077113321) / 2.6490140417448003 + -0.13664977252483368 * ((VY) * (np.power(2,VX)) - 1.8556680327833883) / 36.969035803488815 + -0.10397262871265411 * ((VY) * (np.power(2,AB)) - 0.0025674140019173327) / 0.5269099265396856 + -0.3485304117202759 * ((VY) * (cos(VX/10.0*pi/2.0)) - -0.003216463910703705) / 0.21981376130711971 + -0.21996529400348663 * ((VY) * (cos(BETA/0.44*pi/2.0)) - 0.0012220502335076515) / 0.214245016820561 + -0.45200350880622864 * ((VTHETA) * (sin(VX/10.0*pi/2.0)) - -0.026079669151949817) / 0.3637920253275057 + -0.13080082833766937 * ((np.power(2,VX)) * (tan(BETA/0.44*pi/3.0)) - -2.2649890541305164) / 32.843702597666486 + -0.1437169313430786 * ((np.power(1/2.0,VX)) * (sin(TV/2.1*pi/2.0)) - -0.011021646856018141) / 0.06893767894584851 + -0.159500390291214 * ((sin(BETA/0.44*pi/2.0)) * (cos(VX/10.0*pi/2.0)) - -0.07521730088896078) / 0.5323951275355917 + 0.0017573571531102061

        ATHETA = 0.5181261897087097 * ((VX) * (BETA) - -0.059164368850320524) / 0.616404092345298 + 0.4750429093837738 * ((VX) * (sin(BETA/0.44*pi/2.0)) - -0.1780450110006059) / 1.8442130252282332 + 0.2281590849161148 * ((VY) * (np.power(2,VX)) - 1.8556680327833883) / 36.969035803488815 + 0.21622580289840698 * ((BETA) * (cos(BETA/0.44*pi/2.0)) - -0.015751274569812145) / 0.11001392084333697 + 0.4491049349308014 * ((TV) * (tan(VX/10.0*pi/3.0)) - -0.028058412762629863) / 0.3280168713316988 + 0.39097559452056885 * ((np.power(1/2.0,VX)) * (tan(BETA/0.44*pi/3.0)) - -0.027630559984367788) / 0.2268969856688376 + 0.4242866635322571 * ((cos(BETA/0.44*pi/2.0)) * (tan(BETA/0.44*pi/3.0)) - -0.04171116101243709) / 0.2943814825202627 + -0.3444693088531494 * ((VY) * (cos(BETA/0.44*pi/2.0)) - 0.0012220502335076515) / 0.214245016820561 + -0.2230135202407837 * ((VTHETA) * (sin(AB/6.7*pi/2.0)) - 0.007532114190860106) / 0.14242347115180345 + -0.5136309862136841 * ((VTHETA) * (cos(VX/10.0*pi/2.0)) - -0.03558960931344437) / 0.41623512135178375 + -0.7017034888267517 * ((VTHETA) * (cos(BETA/0.44*pi/2.0)) - -0.024989885213979424) / 0.33482893553899584 + -0.2472326159477234 * ((VTHETA) * (cos(TV/2.1*pi/2.0)) - -0.03627165708598159) / 0.4410768221856903 + -0.6459177136421204 * ((np.power(1/2.0,VX)) * (sin(BETA/0.44*pi/2.0)) - -0.030163300769992137) / 0.2143286512200165 + -0.17217208445072174 * ((cos(VX/10.0*pi/2.0)) * (tan(TV/2.1*pi/3.0)) - -0.033421108224970734) / 0.2963875510800384 + -0.00021069312060717493
    ...
    def NN_poly2_sparse(self, features):
        if isinstance(features, list):
            VX, VY, VTHETA, BETA, AB, TV = features[0]
            VX, VY, VTHETA, BETA, AB, TV = float(VX), float(VY), float(VTHETA), float(BETA), float(AB), float(TV)
        else:
            ...
            AB = features[:, 4]
            TV = features[:, 5]
        AX = 0.19225779175758362 * (AB - 0.06560439433747117) / 0.837672127818587 + 0.2596510052680969 * (np.power(2,AB) - 1.2063352407160353) / 0.7105174540771907 + 0.09356995671987534 * (cos(AB/6.7*pi/2.0) - 0.9815492593309374) / 0.06629921463089376 + 0.16577142477035522 * (tan(AB/6.7*pi/3.0) - 0.00824770448780333) / 0.14978557053838454 + 0.10658609122037888 * ((VX) * (np.power(2,VX)) - 162.55295166394595) / 656.9278623082453 + 0.22890900075435638 * ((VX) * (cos(BETA/0.44*pi/2.0)) - 2.086206577040916) / 1.7625891197618575 + 0.1307162642478943 * ((VX) * (cos(TV/2.1*pi/2.0)) - 2.2877517295054317) / 1.6888098448611537 + 0.4123431444168091 * ((VY) * (VTHETA) - -0.0075517562606167935) / 0.4547007200691509 + 0.13609860837459564 * ((VY) * (BETA) - 0.01930686135404822) / 0.07413735925822189 + 0.10867193341255188 * ((TV) * (tan(AB/6.7*pi/3.0)) - 0.004208543757604824) / 0.08571510690729939 + 0.13030299544334412 * ((np.power(2,AB)) * (cos(BETA/0.44*pi/2.0)) - 0.9252905644261366) / 0.6732057133840967 + 0.27061933279037476 * ((np.power(2,AB)) * (cos(TV/2.1*pi/2.0)) - 1.1059169148482064) / 0.7502383658131054 + 0.08758547902107239 * ((np.power(1/2.0,AB)) * (cos(TV/2.1*pi/2.0)) - 1.2103699084418131) / 4.087102263612652 + 0.15700900554656982 * ((sin(AB/6.7*pi/2.0)) * (cos(TV/2.1*pi/2.0)) - 0.02203966103905764) / 0.1666718588702699 + 0.10131257027387619 * ((cos(VX/10.0*pi/2.0)) * (tan(AB/6.7*pi/3.0)) - 0.003675304873146799) / 0.1154323813869788 + 0.33536824584007263 * ((cos(BETA/0.44*pi/2.0)) * (cos(AB/6.7*pi/2.0)) - 0.7472803531682797) / 0.2511043678086817 + -0.15642313659191132 * (np.power(2,VX) - 24.070437304113003) / 73.46867780096602 + -0.11297154426574707 * (cos(BETA/0.44*pi/2.0) - 0.7617875788004159) / 0.2510784486857777 + -0.09842020273208618 * ((VY) * (tan(BETA/0.44*pi/3.0)) - 0.056866105850210937) / 0.2270559146789968 + -0.19153979420661926 * ((AB) * (np.power(1/2.0,VX)) - 0.002780417668569421) / 0.2644823427681953 + -0.08666213601827621 * ((AB) * (np.power(1/2.0,AB)) - -2.135157198201508) / 27.02672339417089 + -0.0960029885172844 * ((AB) * (tan(AB/6.7*pi/3.0)) - 0.12448425856091473) / 0.5821339463173251 + -0.23926785588264465 * ((np.power(2,AB)) * (np.power(2,AB)) - 1.9600696688406969) / 2.993932741437801 + -0.1966909021139145 * ((np.power(2,AB)) * (sin(VX/10.0*pi/2.0)) - 0.5247879826545464) / 0.6663679076748461 + -0.2287326455116272 * ((sin(VX/10.0*pi/2.0)) * (cos(AB/6.7*pi/2.0)) - 0.3759059155489542) / 0.22705908086289384 + -0.09080126881599426 * ((sin(TV/2.1*pi/2.0)) * (tan(AB/6.7*pi/3.0)) - 0.002685427844477961) / 0.05368985926107068 + -0.15641209483146667 * ((cos(VX/10.0*pi/2.0)) * (cos(BETA/0.44*pi/2.0)) - 0.6621784366881095) / 0.2555961572100748 + -0.13743306696414948 * ((cos(BETA/0.44*pi/2.0)) * (cos(TV/2.1*pi/2.0)) - 0.6944144109841824) / 0.283880552003958 + -0.1430036872625351 * ((cos(AB/6.7*pi/2.0)) * (cos(TV/2.1*pi/2.0)) - 0.8764004424275672) / 0.17286423428443884 + -0.15039542317390442 * ((cos(AB/6.7*pi/2.0)) * (tan(VX/10.0*pi/3.0)) - 0.2988186821761009) / 0.2425852112265329 + 0.0031449489761143923

        AY = 0.2574836313724518 * ((VX) * (VY) - 0.03187135437735826) / 1.6143434321121914 + 0.13881590962409973 * ((VY) * (np.power(1/2.0,VX)) - 0.0007437089541237603) / 0.04139564265858714 + 0.2244095653295517 * ((VY) * (sin(VX/10.0*pi/2.0)) - 0.0030753815466207056) / 0.21484197523316045 + 0.11572103947401047 * ((VY) * (tan(VX/10.0*pi/3.0)) - 0.005144678661245813) / 0.20226677927976314 + 0.18192791938781738 * ((VTHETA) * (np.power(2,AB)) - -0.0232559976392174) / 0.9453828498950136 + 0.22153767943382263 * ((VTHETA) * (cos(VX/10.0*pi/2.0)) - -0.03558960931344437) / 0.41623512135178375 + 0.14042937755584717 * ((VTHETA) * (cos(BETA/0.44*pi/2.0)) - -0.024989885213979424) / 0.33482893553899584 + 0.16233444213867188 * ((BETA) * (cos(TV/2.1*pi/2.0)) - -0.023337366230351705) / 0.17231109199124836 + 0.3537597954273224 * ((np.power(2,VX)) * (sin(BETA/0.44*pi/2.0)) - -2.597021810646166) / 37.75541853730042 + 0.18495206534862518 * ((np.power(1/2.0,VX)) * (tan(TV/2.1*pi/3.0)) - -0.008128013810010258) / 0.05381832431948496 + 0.14055421948432922 * ((sin(VX/10.0*pi/2.0)) * (sin(BETA/0.44*pi/2.0)) - -0.02551175347595281) / 0.2586397131882021 + 0.19949603080749512 * ((sin(VX/10.0*pi/2.0)) * (tan(BETA/0.44*pi/3.0)) - -0.02448072465955652) / 0.2542822057050383 + 0.14044322073459625 * ((sin(VX/10.0*pi/2.0)) * (tan(AB/6.7*pi/3.0)) - 0.007619796280901655) / 0.09543342111646996 + 0.18251483142375946 * ((sin(TV/2.1*pi/2.0)) * (cos(VX/10.0*pi/2.0)) - -0.04287448217319001) / 0.3447850390439351 + 0.1364893615245819 * ((cos(BETA/0.44*pi/2.0)) * (tan(TV/2.1*pi/3.0)) - -0.028359014070580238) / 0.2458638828706388 + -0.14313140511512756 * (VY - 0.0006739826858158322) / 0.3073995994274385 + -0.13878418505191803 * (VTHETA - -0.04802685383934488) / 0.5524821241332127 + -0.14292022585868835 * ((VX) * (VTHETA) - -0.19631195879670152) / 2.706649563739692 + -0.21054430305957794 * ((VX) * (TV) - -0.2317972077113321) / 2.6490140417448003 + -0.166886568069458 * ((VY) * (np.power(2,VX)) - 1.8556680327833883) / 36.969035803488815 + -0.11372514814138412 * ((VY) * (np.power(2,AB)) - 0.0025674140019173327) / 0.5269099265396856 + -0.35168716311454773 * ((VY) * (cos(VX/10.0*pi/2.0)) - -0.003216463910703705) / 0.21981376130711971 + -0.15261578559875488 * ((VY) * (cos(BETA/0.44*pi/2.0)) - 0.0012220502335076515) / 0.214245016820561 + -0.17840532958507538 * ((VTHETA) * (sin(VX/10.0*pi/2.0)) - -0.026079669151949817) / 0.3637920253275057 + -0.18970613181591034 * ((VTHETA) * (tan(VX/10.0*pi/3.0)) - -0.024984063549927302) / 0.3369705692353521 + -0.18327775597572327 * ((BETA) * (tan(VX/10.0*pi/3.0)) - -0.006927802459959079) / 0.07331567891700316 + -0.13258035480976105 * ((AB) * (sin(VX/10.0*pi/2.0)) - 0.05441040148595401) / 0.5490728695560814 + -0.40471184253692627 * ((np.power(2,VX)) * (tan(BETA/0.44*pi/3.0)) - -2.2649890541305164) / 32.843702597666486 + -0.30716684460639954 * ((np.power(1/2.0,VX)) * (sin(TV/2.1*pi/2.0)) - -0.011021646856018141) / 0.06893767894584851 + -0.22075031697750092 * ((sin(BETA/0.44*pi/2.0)) * (cos(VX/10.0*pi/2.0)) - -0.07521730088896078) / 0.5323951275355917 + 0.0012592626735568047

        ATHETA = 0.13941696286201477 * (sin(BETA/0.44*pi/2.0) - -0.08303234071430579) / 0.5913993436400382 + 0.1890529990196228 * ((VX) * (BETA) - -0.059164368850320524) / 0.616404092345298 + 0.44439181685447693 * ((VX) * (sin(BETA/0.44*pi/2.0)) - -0.1780450110006059) / 1.8442130252282332 + 0.185264453291893 * ((VX) * (tan(TV/2.1*pi/3.0)) - -0.13127455566371446) / 1.592128475724603 + 0.16974057257175446 * ((VY) * (np.power(2,VX)) - 1.8556680327833883) / 36.969035803488815 + 0.11706582456827164 * ((VY) * (tan(VX/10.0*pi/3.0)) - 0.005144678661245813) / 0.20226677927976314 + 0.13069036602973938 * ((BETA) * (AB) - 0.006754610408016997) / 0.16145230468338204 + 0.33632972836494446 * ((BETA) * (cos(BETA/0.44*pi/2.0)) - -0.015751274569812145) / 0.11001392084333697 + 0.13214461505413055 * ((BETA) * (tan(VX/10.0*pi/3.0)) - -0.006927802459959079) / 0.07331567891700316 + 0.26921704411506653 * ((TV) * (tan(VX/10.0*pi/3.0)) - -0.028058412762629863) / 0.3280168713316988 + 0.3484593629837036 * ((np.power(1/2.0,VX)) * (tan(BETA/0.44*pi/3.0)) - -0.027630559984367788) / 0.2268969856688376 + 0.14444580674171448 * ((sin(VX/10.0*pi/2.0)) * (sin(VX/10.0*pi/2.0)) - 0.20709508370742272) / 0.23403293537837136 + 0.2895820438861847 * ((sin(VX/10.0*pi/2.0)) * (sin(BETA/0.44*pi/2.0)) - -0.02551175347595281) / 0.2586397131882021 + 0.21154192090034485 * ((sin(BETA/0.44*pi/2.0)) * (tan(VX/10.0*pi/3.0)) - -0.020904650196453264) / 0.22045920739339014 + 0.133474200963974 * ((sin(BETA/0.44*pi/2.0)) * (tan(TV/2.1*pi/3.0)) - 0.15590299154558995) / 0.2239495699550572 + 0.3011733293533325 * ((cos(BETA/0.44*pi/2.0)) * (tan(BETA/0.44*pi/3.0)) - -0.04171116101243709) / 0.2943814825202627 + 0.188899964094162 * ((tan(VX/10.0*pi/3.0)) * (tan(TV/2.1*pi/3.0)) - -0.01600544101398789) / 0.1995590527023137 + -0.1930362433195114 * (VTHETA - -0.04802685383934488) / 0.5524821241332127 + -0.1793668568134308 * (tan(VX/10.0*pi/3.0) - 0.30891582763733216) / 0.2592753221105449 + -0.43468308448791504 * ((VY) * (cos(BETA/0.44*pi/2.0)) - 0.0012220502335076515) / 0.214245016820561 + -0.2329595386981964 * ((VTHETA) * (sin(AB/6.7*pi/2.0)) - 0.007532114190860106) / 0.14242347115180345 + -0.45615917444229126 * ((VTHETA) * (cos(VX/10.0*pi/2.0)) - -0.03558960931344437) / 0.41623512135178375 + -0.8798017501831055 * ((VTHETA) * (cos(BETA/0.44*pi/2.0)) - -0.024989885213979424) / 0.33482893553899584 + -0.10334508121013641 * ((VTHETA) * (cos(TV/2.1*pi/2.0)) - -0.03627165708598159) / 0.4410768221856903 + -0.10696464776992798 * ((TV) * (sin(BETA/0.44*pi/2.0)) - 0.2793202086486653) / 0.3794309480071819 + -0.20132213830947876 * ((np.power(2,VX)) * (tan(TV/2.1*pi/3.0)) - -2.4759847786317444) / 46.232339550042035 + -0.598541796207428 * ((np.power(1/2.0,VX)) * (sin(BETA/0.44*pi/2.0)) - -0.030163300769992137) / 0.2143286512200165 + -0.2135070562362671 * ((cos(VX/10.0*pi/2.0)) * (tan(TV/2.1*pi/3.0)) - -0.033421108224970734) / 0.2963875510800384 + -0.09456044435501099 * ((cos(BETA/0.44*pi/2.0)) * (tan(TV/2.1*pi/3.0)) - -0.028359014070580238) / 0.2458638828706388 + -0.2567893862724304 * ((tan(VX/10.0*pi/3.0)) * (tan(BETA/0.44*pi/3.0)) - -0.01987165703715103) / 0.2126161286473956 + 0.0002547540934756398
