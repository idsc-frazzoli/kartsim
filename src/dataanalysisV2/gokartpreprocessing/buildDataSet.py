...
#                 preproParams[testDay][logNr]['MH BETA'] = 1
            ...
            if kartData['steer position cal']['data'][1][int(lenSteerCmd/10):int(lenSteerCmd/10*9)].count(0)/len(kartData['steer position cal']['data'][1][int(lenSteerCmd/10):int(lenSteerCmd/10*9)]) > 0.05:
                statusInfo = statusInfo + 'steering pos cal data: too many zeros...,  '
                preproParams[testDay][logNr]['steer position cal'] = 0
                preproParams[testDay][logNr]['MH BETA'] = 0
            elif np.abs(np.mean(kartData['steer position cal']['data'][1])) < 0.01 and np.std(kartData['steer position cal']['data'][1]) < 0.1:
