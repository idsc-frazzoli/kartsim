...
def main():
    ...
    for index in range(len(pklfiles)):
        try:
            rawdataframe = dIO.getPKL(pklfiles[index][0])
        except:
            print('EmptyDataError: could not read data from file ', pklfiles[index][0])
            raise
        ...
        for j in range(2):
            try:
                simdataframe = dIO.getCSV(csvfiles[index*2+j][0])
            except:
                print('EmptyDataError: could not read data from file ', csvfiles[index*2+j][0])
                raise
            ...
            print('Evaluation for ', csvfiles[index * 2 + j][1], 'vs', pklfiles[index][1])
            print(rmse[j])
            print('Overall score: ', np.sum(rmse[j]), '\n')


        #generating results page
        pdffilepath = pathsimdata + '/' + pklfiles[index][1][:-19] + '_evaluation_plots.pdf'
        ...
        for k in range(2):
            ...
            infotext1 +='Evaluation for ' + csvfiles[index * 2 + k][1] + ' vs ' + pklfiles[index][1] + '\n'
            infotext1 += 'Parameter' + '\n'
        ...
        pdf.savefig()
        # print('Evaluation for ' + csvfiles[index * 2 + k][1] + ' vs ' + pklfiles[index][1] + '\n')
        # print(rmse)
        ...
        #             the_file.write('Evaluation for ' + csvfiles[index*2+j][1] + ' vs ' + pklfiles[index][1] + '\n')
