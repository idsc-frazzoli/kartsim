...
def marc_vehiclemodel (X,W):
    ...
    acc = np.hstack((np.array([accX]).transpose(),np.array([accY]).transpose(),np.array([accRot]).transpose()))
    return acc
...
def pymodelDx(X, W, param):
    global B1, B2, C1, C2, D1, D2, reg, offset_vx
    # offset_vx, offset_vy, offset_vtheta = W
    # offset_vx = W
    B1, C1, D1, B2, C2, D2, Ic = W
    ...
    f1_velx = np.sum(np.multiply(rotmat(BETA)[0],
                                 np.array([VELX, VELY + l1 * VELROTZ])), axis=0)
    f1_vely = np.sum(np.multiply(rotmat(BETA)[1],
                                 np.array([VELX, VELY + l1 * VELROTZ])), axis=0)
    f1y = simplefaccy(f1_vely, f1_velx)

    F1x = np.sum(np.multiply(rotmat(-BETA)[0], np.array([np.zeros(len(f1y)), f1y])),
                 axis=0) * f1n
    F1y = np.sum(np.multiply(rotmat(-BETA)[1], np.array([np.zeros(len(f1y)), f1y])),
                 axis=0) * f1n
    F2x = AB
