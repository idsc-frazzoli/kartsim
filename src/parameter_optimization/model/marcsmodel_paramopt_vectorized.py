...
def jacobian_of_pymodelDx(X, W, param):
    ...
    Jacobian = np.array([[F1x, np.zeros(len(F1x)), np.zeros(len(F1x)), np.zeros(len(F1x))],
                [F1y, F2y1 + F2y2, np.zeros(len(F1y)), np.zeros(len(F1y))],
                [F1y * l1 / Ic, F2y * l2 / Ic, TV / Ic, -(TVTrq + D1 * F1y * l1 - D2 * F2y * l2) / np.square(Ic)]]).transpose()
